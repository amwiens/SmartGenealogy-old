...
namespace SmartGenealogy.Maui.Navigation;
...
public class AppNavigator : IAppNavigator
{
    ...
    public Task NavigateAsync(string target, bool animated = false, bool inNewWindow = false, object args = null)
    {
        ...
        if (inNewWindow && AllowsInNewWindow())
        {
            return Task.Run(() =>
            {
                var page = Routing.GetOrCreateContent(target, _serviceProvider) as Page;
